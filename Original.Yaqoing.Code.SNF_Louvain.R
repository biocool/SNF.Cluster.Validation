...
SNF_Louvain <- function(dat, ROI_var, clinic_var) {
  ...
  fviz_cluster(
    object = list(data = W, cluster = dat$index), geom = "point",
  ...
  for (x in 1:dim(ind)[1]) {
    snf_matrix[x, 3:5] <- cbind(
      colnames(W)[ind[x, "col"]],
  ...
  snf_matrix$name <- paste(as.character(snf_matrix$Source),
    as.character(snf_matrix$Target),
    sep = "."
  )
  ...
  G1 <- graph_from_adjacency_matrix(gg,
